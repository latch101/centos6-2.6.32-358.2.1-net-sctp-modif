					...
					sctp_cmd_seq_t *commands)
{
	struct sctp_chunk *chunk = arg;
	int error;
 //   printk("Data received assocId=%d\n", asoc->assoc_id);
	if (!sctp_vtag_verify(chunk, asoc)) {
...
{
	struct sctp_chunk *chunk = arg;
	struct sctp_association *shadow = (struct sctp_association *)asoc;
...
//	__u16 dup;
 ...
 //   dup  = ntohl(sackh->num_dup_tsns);
	...
	if(shadow->bkup_sack)
	{
		if(ctsn != shadow->ctsn_ack_point)
		{
		   //unsigned int len = asoc->ssnmap->out.len;
		   int ssn = (ctsn - shadow->next_tsn)+1;
           //int ssn = ctsn - shadow->ctsn_ack_point;

           printk("SCTP HO: Adjusting SACK tsn rx %d nxt %d cap %d delta %d\n",
        		   ctsn, shadow->next_tsn, shadow->ctsn_ack_point, ssn);
           if(ssn > -1)
           {
              if(shadow->ssnmap->out.ssn[1])
              {
                 shadow->ssnmap->out.ssn[1] += ssn;
              }
              else
              {
                 shadow->ssnmap->out.ssn[0] += ssn;
              }
           }
           //if(dup)
           //{
        	//   ctsn--;
           //}
           shadow->ctsn_ack_point = ctsn;
           shadow->next_tsn = ctsn+1;
		}
		shadow->bkup_sack = 0;
	}
			 ...
			 sctp_cmd_seq_t *commands)
{
	...
	struct sctp_ssnmap *ssnm =  asoc->ssnmap;
	...
	if(asoc->bkup_data)
	{
		printk("SCTP HO:adjusting data tsn\n");
		ssnm->in.ssn[sid] = ntohs(data_hdr->ssn);
		map->cumulative_tsn_ack_point = tsn - 1;
		map->base_tsn = tsn;
		shadow->bkup_data = 0;
	}
	...
	if (tmp < 0)
	{
		 ...
		 */
		printk("sctp_tsnmap_check error tsn too high %d\n", tmp);
		return SCTP_IERROR_HIGH_TSN;
	} else if (tmp > 0) {
		/* This is a duplicate.  Record it.  */
		printk("sctp_tsnmap_check error duplicate %d\n", tmp);
